...
Note: These tests require a configured LLM (GROQ_API_KEY, GEMINI_API_KEY,
OPENAI_API_KEY, or ANTHROPIC_API_KEY) because classify_node and answer_node
use real LLM calls. The project ``.env`` is loaded in ``tests/conftest.py``.
"""
...
pytestmark = [
    ...
    pytest.mark.skipif(
        ...
        reason="No LLM API key configured (set GROQ_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY)",
    ),
]
